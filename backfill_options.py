...
건드리는 값은 options와 price뿐이다. status/published/deadline은 손대지 않는다 —
이미 검수가 끝난 공구가 백필 때문에 화면에서 사라지거나 다시 검수 목록에 오르면
안 되기 때문이다. price는 옵션이 잡혔을 때 가장 싼 세트로만 낮춘다(화면이 "N원부터"로
쓰기 때문이며, 원래 값이 더 싸면 그대로 둔다).
...
def main():
    ...
    for i, p in enumerate(targets, 1):
        ...
        if not args.dry_run:
            p["options"] = opts
            if not old_price or old_price > cheapest:
                p["price"] = cheapest
