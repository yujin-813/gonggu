import type { Metadata } from 'next'
import { visiblePosts, evergreenPosts, landingCopy } from '@/lib/landing'
import { landingMetadata, LandingPage } from '@/lib/landingPage'

export const dynamic = 'force-dynamic'

function load() {
  const posts = evergreenPosts(visiblePosts())
  return { posts, copy: landingCopy('evergreen', posts.length) }
}

export function generateMetadata(): Metadata {
  const { posts, copy } = load()
  return landingMetadata(copy, posts)
}

export default function EvergreenPage() {
  const { posts, copy } = load()
  return <LandingPage copy={copy} posts={posts} />
}
