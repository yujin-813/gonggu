...
| 코드 규모 | `lib/` 3,416줄 · `app/admin` + `components/` 6,891줄 · 파이썬 3,019줄 |
...
- **「공구 제안 · 입점 문의」**(`/propose`, 전 페이지 하단, 관리자 화면 제외) — 인플루언서·브랜드가
  제휴를 제안할 창구. `/request`(공구 하나 등록 요청)와 달리 구매 링크·가격이 없어도 된다.
  제출은 `data/inquiries.json`에 먼저 저장하고(진실의 원천), SMTP가 설정돼 있으면 메일도 보낸다 —
  메일이 실패해도 문의는 안 사라진다. 관리자 **「제휴 문의」** 탭에서 확인·처리 표시한다
