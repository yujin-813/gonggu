...
| 코드 규모 | `lib/` 3,305줄 · `app/admin` + `components/` 6,144줄 · 파이썬 3,019줄 |
...
- **성장 목표** — 일 방문자 기준 6단계(기본 150→300→500→1,000→3,000→10,000, 관리자에서 수정 가능,
  `data/growth_goals.json`). 현재 단계는 최근 7일 평균으로 판정(요일 편차 때문에 하루 값 안 씀)
...
- - **데이터** 탭 — 개별 로그 위에 **요약 3숫자**(검색 유입 · 검색→상세 비율 · 상세→구매처클릭 비율,
