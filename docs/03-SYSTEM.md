...
| 코드 규모 | `lib/` 3,415줄 · `app/admin` + `components/` 7,083줄 · 파이썬 3,059줄 |
...
- - **마감 공구의 추천 목록은 관련도 등급을 섞지 않는다.** 제목 낱말 겹침·같은 브랜드면
  "이런 건 어때요?", 같은 인플루언서뿐이면 "같은 인플루언서의 다른 공구", 카테고리뿐이면
  "지금 진행 중인 다른 OO 공구". 카테고리는 인플루언서 성격을 따라가서(곰탕이 `kids`)
...
### 3. 판정 대기가 45% ⚠️ (오픈 예정을 빼면 10%)

고객에게 보이는 공구의 절반 가까이가 판정을 못 받는다. 다만 이 숫자는 그대로 읽으면 안 된다 —
`D-029`로 공개한 **오픈 예정 47건 중 46건이 가격이 없어서**(2026-08-25 기준, 1건은 실제로
열려 채워졌다) 자동으로 pending에 들어간다. 아직 안 열린 공구라 판정할 수 없는 게 정상이고,
관리자 작업 목록(채우기 「미확인」·수익화 경고)에서는 빼고 센다.
새로 들어오는 것일수록 나쁘다 — 8월 수집분은 절반 가까이가 판정을 못 받는다. 문제 1(비교가 수집 중단)이
고객 화면에서 드러나는 자리다.

**근거:** 2026-08-24 측정(`scripts/check-docs.js`) — 보이는 공구 120건 중 pending 54건. 그중 47건이 오픈 예정이라, 실제로 채워야 할 것은 **73건 중 7건**이다.
