...
| 코드 규모 | `lib/` 3,426줄 · `app/admin` + `components/` 6,852줄 · 파이썬 3,019줄 |
...
- **꿀딜 확산 후보** — 진행 중+가격비교 완료+꿀딜/괜찮딜+인플루언서 계정 있는 상품만, 할인폭→
  반응→미게시 순 정렬. 1:1 DM은 안 쓴다(D-057) — 행마다 공유 이미지(`/api/og/deal/[id]`)·
  추적링크 복사, 상단에 상위 5개를 묶은 "우리 채널용" 배치 문구 복사. 상태는
...
- - **데이터** 탭 — 맨 위 **구매 기록**(`data/purchase_log.json`) — 실제 구매를 관리자가 손으로
  남긴다(자동 감지 없음). 목표는 금액이 아니라 누적 **구매 건수**(1→5→20, `D-059`). 3건부터
  마감상품 대체구매 vs 진행중 가격비교 건수를 보여준다. 그 아래 **성장 목표** 카드(일 방문자
  기준 6단계, 기본 150→300→500→1,000→3,000
  →10,000, 관리자에서 수정 가능, `data/growth_goals.json`. 현재 단계는 요일 편차 때문에 하루 값
  대신 최근 7일 평균으로 판정)에 「공구 클릭(7일)」·「구매처(제휴) 클릭(7일)」 카드가 나란히 있다 —
  둘 다 **지금 공개 중인 상품만**(`isPagePublic`) 센다. 「수익화 현황 → 상품별로 보기」 표와
  모집단을 맞추지 않으면 제외된 상품의 옛 클릭이 합계에만 섞여 숫자가 안 맞아 보인다(`D-062`).
  카드를 누르면 수익화 현황으로 이동한다. 그 아래 **요약 3숫자**(검색 유입 · 검색→상세 비율 · 상세→구매처클릭 비율,
  14일 집계)를 먼저 보여주고, 그 아래 최근 방문을 **한 줄 가로 타임라인**으로 본다.
