...
| 코드 규모 | `lib/` 3,367줄 · `app/admin` + `components/` 6,402줄 · 파이썬 3,019줄 |
...
- **인플루언서 확산 후보** — 진행 중+가격비교 완료+꿀딜/괜찮딜+인플루언서 계정 있는 상품만, 할인폭→
  반응→미연락 순 정렬. 행마다 공유 이미지(`/api/og/deal/[id]`)·DM 문구 복사·추적링크 복사, 상태는
  미전달/전달완료/공유확인/유입발생(`Post.outreach_status`, 관리자 전용) (`D-056`)
