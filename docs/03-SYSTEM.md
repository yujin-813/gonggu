...
| 코드 규모 | `lib/` 3,107줄 · `app/admin` + `components/` 5,723줄 · 파이썬 2,986줄 |
...
`D-029`로 공개한 **오픈 예정 47건이 전부 가격이 없어서** 자동으로 pending에 들어간다. 아직 안
열린 공구라 판정할 수 없는 게 정상이고, 관리자 작업 목록(채우기 「미확인」·수익화 경고)에서는
빼고 센다.
